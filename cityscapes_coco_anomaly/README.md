...
- Python 3.10 <= version <= 3.12
- Required Python dependencies installed
- Cityscapes and COCO datasets available locally, or dataset download enabled in the configuration file

---

### Running the Dataset Generation

The dataset is generated by running the `build_dataset.py` script directly.

From the repository root, execute:

```
python synthgen/build_dataset.py --config configs/synth_dataset_v1.yaml --splits train,val,test
```

The `--splits` argument accepts a comma-separated list of dataset splits.

For example, to generate only the training split:

```
python synthgen/build_dataset.py --config configs/synth_dataset_v1.yaml --splits train
```

---

### Optional: Dataset Download / Preparation

If dataset download or preparation is enabled in the YAML configuration file, the script can automatically download and prepare the required datasets.

Run:

```
python synthgen/build_dataset.py --config configs/synth_dataset_v1.yaml --splits train,val,test --prepare_datasets
```

To force a re-download:

```
python synthgen/build_dataset.py --config configs/synth_dataset_v1.yaml --splits train,val,test --prepare_datasets --force_download
```
...
All generated data are written under the directory specified by `output_root` in the configuration file.

The output directory is organized by dataset split and includes:
- RGB images
- pixel-level anomaly ground truth
- instance-level anomaly masks and labels
- a `manifest.jsonl` file recording how each sample was generated

---

### Notes

- The output directory should be cleared between runs to avoid mixing datasets generated with different configurations.
- Dataset generation is deterministic: running the script multiple times with the same configuration produces identical results.
